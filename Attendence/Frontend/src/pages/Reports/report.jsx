  ...
  const handleDownload = async (type) => {
    try {
      ...
      if (type === "absent") {
        ...
        apiEndpoint = `/abs-report?year=${slotYear.value}&date=${formatDate(absentSlotDate)}&slot=${slot.value || "All"}`;
        fileName = `studentsAbsent-${slotYear.value}-${formatDate(absentSlotDate)}.xlsx`;
      } else if (type === "present-slot") {
        apiEndpoint = `/pres-report?year=${presentSlotYear.value}&date=${formatDate(presentSlotDate)}&slot=${presentSlot.value || "All"}`;
        fileName = `studentsPresent-${presentSlotYear.value}-${formatDate(presentSlotDate)}-${presentSlot.label || "All"}.xlsx`;
        ...
        fileName = `ConsolidateReport-${conYear?.value || "All"}-${formatDate(consolidateFDate)}-to-${formatDate(consolidateTDate)}.xlsx`;
      }
      ...
      if (type === "absent-slot") {
        const totalAbsentRow = [`Total Absent Students: ${data.total_absent_students}`];
        const studentHeader = ["student_name", "register_number", "mail", "mentor_name", "slot"];
        const studentRows = data.students.map(({ student_name, register_number, mail, mentor_name, slot }) => [
          student_name,
          register_number,
          mail,
          mentor_name,
