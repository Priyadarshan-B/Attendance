...
exports.getAttendanceByDate = async (req, res) => {
  ...
  CASE 
    WHEN 
      COUNT(DISTINCT CASE WHEN ts.session = 'FN' THEN ts.id END) <= 
      (COUNT(DISTINCT CASE WHEN ts.session = 'FN' AND COALESCE(r.status, '0') = '1' THEN ts.id END) + 1)
    THEN 'PR'
    ELSE 'AB'
  END AS forenoon_status,
  CASE 
    WHEN 
      COUNT(DISTINCT CASE WHEN ts.session = 'AN' THEN ts.id END) <= 
      (COUNT(DISTINCT CASE WHEN ts.session = 'AN' AND COALESCE(r.status, '0') = '1' THEN ts.id END) + 1)
    THEN 'PR'
    ELSE 'AB'
...
FROM 
  students s
LEFT JOIN 
  time_slots ts ON s.year = ts.year
LEFT JOIN 
  re_appear r ON s.id = r.student AND ts.id = r.slot AND DATE(r.att_session) = '${formatted_date}'
WHERE 
   s.year = ${yearCondition}
GROUP BY 
  s.id, s.name, s.register_number, s.gmail;
    ...
    console.error("Error fetching slot with_exemp only attendance status", err);
    res
      .status(500)
      .json({ error: "Error fetching slot with_exemp only attendance status" });
  }
};
