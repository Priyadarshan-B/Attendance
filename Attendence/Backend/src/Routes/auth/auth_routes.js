...
// Callback route
router.get("/google/callback", passport.authenticate("google", { failureRedirect: `${process.env.CLIENT_URL}/login` }), async function (req, res) {
    ...
    try {
        ...
        const response = await axios.get(`${process.env.API_URL}/auth/resources?role=${role}`);
...
const generateToken = (user) => {
    ...
    return jwt.sign(
        { userId: user.id, name: user.name, roll: user.register_number, role: user.role_id, id: user.id, gmail: user.gmail, profile: user.profilePhoto },
        JWT_SECRET,
        { expiresIn: '1h' }
    );
};
